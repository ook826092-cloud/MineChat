                ...
                message.copy(
                    parts = message.parts.flatMap { part ->
                        if (part is UIMessagePart.Text && THINKING_REGEX.containsMatchIn(part.text)) {
                            ...
                            Log.d("DupRender", "ThinkTag.visualTransform SPLIT: alreadyHasReasoning=${message.hasPart<UIMessagePart.Reasoning>()} reasoning=[${reasoning.take(80)}]")
                            listOf(
                    ...
                    parts = message.parts.flatMap { part ->
                        if (part is UIMessagePart.Text && THINKING_REGEX.containsMatchIn(part.text)) {
                            ...
                            val reasoning =
                                THINKING_REGEX.find(part.text)?.groupValues?.getOrNull(1)?.trim()
                                    ?: ""
                            Log.d("DupRender", "ThinkTag.onFinish SPLIT: alreadyHasReasoning=${message.hasPart<UIMessagePart.Reasoning>()} reasoning=[${reasoning.take(80)}]")
                            listOf(
                                UIMessagePart.Reasoning(
                                    reasoning = reasoning,
                                    createdAt = message.createdAt.toInstant(timeZone = TimeZone.currentSystemDefault()),
