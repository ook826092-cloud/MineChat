package cn.mine.minestars.data.provider
...
class WorkspaceDocumentsProvider : DocumentsProvider() {
    ...
    override fun copyDocument(sourceDocumentId: String, targetParentDocumentId: String): String {
        ...
        require(!target.isRoot) { "Cannot copy to root" }
        manager().ensureWorkspace(target.root)
        val srcFile = resolveFile(source.root, source.relPath)
        val targetDir = resolveFile(target.root, target.relPath)
        require(targetDir.isDirectory) { "Target parent is not a directory" }
        val dest = uniqueChild(targetDir, srcFile.name)
        if (srcFile.isDirectory) {
            require(!dest.path.startsWith(srcFile.canonicalFile.path + File.separator)) {
    ...
    override fun moveDocument(
        ...
        val source = parseDocId(sourceDocumentId)
        val target = parseDocId(targetParentDocumentId)
        require(!target.isRoot) { "Cannot move to root" }
        manager().ensureWorkspace(target.root)
        val srcFile = resolveFile(source.root, source.relPath)
        val targetDir = resolveFile(target.root, target.relPath)
        require(targetDir.isDirectory) { "Target parent is not a directory" }
        val dest = uniqueChild(targetDir, srcFile.name)
        val renamed = if (source.root == target.root) {
    ...
    companion object {
        private const val ROOT_ID = "minechat_workspaces"
