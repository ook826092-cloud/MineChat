...
import cn.mine.common.cache.LruCache
import cn.mine.common.cache.SingleFileCacheStore
...
object OcrTransformer : InputMessageTransformer, KoinComponent {
    ...
    suspend fun performOcr(part: UIMessagePart.Image): String = runCatching {
        ...
        val settings = get<AiSettingsStore>().flow.value
        val providerDAO = get<ProviderDAO>()
        val providers = providerDAO.getAll().toProviderSettings()
        val selectionDAO = get<cn.mine.minestars.data.db.dao.ModelSelectionDAO>()
        val ocrModelId = selectionDAO.get()?.ocrModelId?.let { runCatching { kotlin.uuid.Uuid.parse(it) }.getOrNull() }
        val model = ocrModelId?.let { providers.findModelById(it) } ?: return "[Image]"
        val providerSetting = model.findProvider(providers) ?: return "[Image]"
