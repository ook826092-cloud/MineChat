package cn.mine.minestars.data.files
...
class SkillManager(
    ...
    suspend fun deleteSkill(name: String): Boolean = withContext(Dispatchers.IO) {
        ...
        if (deleted) {
            val assistants = assistantDAO.getAll().map { it.toModel() }
            assistants.filter { it.enabledSkills.contains(name) }.forEach { assistant ->
                assistantDAO.insert(
                    assistant.copy(enabledSkills = assistant.enabledSkills - name).toEntity()
                )
            }
        }
        deleted
