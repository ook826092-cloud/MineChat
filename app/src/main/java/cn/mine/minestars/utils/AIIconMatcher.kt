package cn.mine.minestars.utils
...
fun computeAIIconByName(name: String): String? {
    ...
    val result = when {
        ...
        PATTERN_MINECHAT.containsMatchIn(lowerName) -> "minechat.svg"
...
/**
 * 匹配提供商名称到本地 drawable 品牌图标资源 ID
 * 优先使用本地 PNG 图标，如果不存在则返回 null（由 AutoAIIcon 回退到 SVG asset）
 */
fun computeBrandDrawableResId(name: String): Int? {
    val lowerName = name.lowercase()
    return when {
        PATTERN_OPENAI.containsMatchIn(lowerName) -> R.drawable.brand_openai
        PATTERN_GEMINI.containsMatchIn(lowerName) -> R.drawable.brand_gemini
        PATTERN_GOOGLE.containsMatchIn(lowerName) -> R.drawable.brand_gemini
        PATTERN_CLAUDE.containsMatchIn(lowerName) -> R.drawable.brand_claude
        PATTERN_ANTHROPIC.containsMatchIn(lowerName) -> R.drawable.brand_claude
        PATTERN_DEEPSEEK.containsMatchIn(lowerName) -> R.drawable.brand_deepseek
        PATTERN_GROK.containsMatchIn(lowerName) -> R.drawable.brand_grok
        PATTERN_QWEN.containsMatchIn(lowerName) -> R.drawable.brand_qwen
        PATTERN_DOUBAO.containsMatchIn(lowerName) -> R.drawable.brand_doubao
        PATTERN_OPENROUTER.containsMatchIn(lowerName) -> R.drawable.brand_openrouter
        PATTERN_ZHIPU.containsMatchIn(lowerName) -> R.drawable.brand_zhipu
        PATTERN_MISTRAL.containsMatchIn(lowerName) -> R.drawable.brand_mistral
        PATTERN_META.containsMatchIn(lowerName) -> R.drawable.brand_meta
        PATTERN_HUNYUAN.containsMatchIn(lowerName) -> R.drawable.brand_hunyuan
        PATTERN_BYTEDANCE.containsMatchIn(lowerName) -> R.drawable.brand_spark
        PATTERN_ALIYUN.containsMatchIn(lowerName) -> R.drawable.brand_spark
        PATTERN_KIMI.containsMatchIn(lowerName) -> R.drawable.brand_kimi
        PATTERN_MOONSHOT.containsMatchIn(lowerName) -> R.drawable.brand_kimi
        PATTERN_MINIMAX.containsMatchIn(lowerName) -> R.drawable.brand_minimax
        PATTERN_XAI.containsMatchIn(lowerName) -> R.drawable.brand_xai
        PATTERN_GROQ.containsMatchIn(lowerName) -> R.drawable.brand_groq
        PATTERN_NVIDIA.containsMatchIn(lowerName) -> R.drawable.brand_nvidia
        PATTERN_SILLICON_CLOUD.containsMatchIn(lowerName) -> R.drawable.brand_siliconcloud
        PATTERN_STEP.containsMatchIn(lowerName) -> R.drawable.brand_stepfun
        PATTERN_COHERE.containsMatchIn(lowerName) -> R.drawable.brand_chatglm
        PATTERN_PERPLEXITY.containsMatchIn(lowerName) -> R.drawable.brand_spark
        PATTERN_MINECHAT.containsMatchIn(lowerName) -> R.drawable.brand_openai
        else -> null
    }
}

private val PATTERN_MINECHAT = Regex("rikka|auto")
