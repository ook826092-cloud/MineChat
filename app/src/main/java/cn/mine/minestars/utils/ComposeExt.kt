...
fun TextUnit.toDp(): Dp = with(LocalDensity.current) {
    when (this@toDp.type) {
        TextUnitType.Sp -> this@toDp.toDp()
        TextUnitType.Em -> {
            val baseSp = LocalTextStyle.current.fontSize.let {
                if (it.type != TextUnitType.Unspecified && it.type == TextUnitType.Sp) it
                else 16.sp
            }
            (baseSp.value * this@toDp.value).sp.toDp()
        }
        else -> 0.dp
    }
}
