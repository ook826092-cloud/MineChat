...
import cn.mine.tts.model.PlaybackState
...
import cn.mine.minestars.utils.stripMarkdown
import cn.mine.tts.model.TTSResponse
import cn.mine.tts.provider.TTSManager
import cn.mine.tts.provider.TTSProviderSetting
import cn.mine.tts.controller.TtsController
...
fun rememberCustomTtsState(): CustomTtsState {
    ...
    val ttsProviderDAO = koinInject<TTSProviderDAO>()
    ...
    LaunchedEffect(aiSettings.selectedTTSProviderId) {
        val selectedProvider = ttsProviderDAO.getAll()
            .find { it.id == aiSettings.selectedTTSProviderId.toString() }
            ?.let { JsonInstant.decodeFromString<TTSProviderSetting>(it.config) }
        ttsState.updateProvider(selectedProvider)
    }
...
private class CustomTtsStateImpl(
    ...
    private val controller by lazy { cn.mine.tts.controller.TtsController(context, ttsManager) }
