...
private fun AssistantLocalToolContent(
    ...
    Column(
        ...
        CardGroup {
            ...
            item(
                headlineContent = {
                    Text("萌娘百科 (Moegirl Wiki)")
                },
                supportingContent = {
                    Text("搜索ACG、二次元、动漫、游戏相关条目 — 萌娘百科专用搜索")
                },
                trailingContent = {
                    Switch(
                        checked = assistant.localTools.contains(LocalToolOption.MoegirlWiki),
                        onCheckedChange = { toggleLocalTool(LocalToolOption.MoegirlWiki, it) }
                    )
                }
            )
        }
