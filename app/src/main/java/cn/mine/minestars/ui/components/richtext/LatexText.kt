...
fun splitLatex(
    ...
    color: Int,
    background: Int,
    availableWidth: Float = Float.MAX_VALUE,
): List<JLatexMathDrawable> {
    val processed = processLatex(latex)
    return runCatching {
        JLatexMathSplitter.split(processed, fontSize, availableWidth, color)
    }.onFailure {
        it.printStackTrace()
    }.getOrNull() ?: listOfNotNull(
        getLatexDrawable(latex, fontSize, color, background)
    )
}

@Composable
fun LatexDrawable(
    drawables: List<JLatexMathDrawable>,
    modifier: Modifier = Modifier,
) {
    val density = LocalDensity.current
