package cn.mine.minestars.ui.pages.log
...
import cn.mine.minestars.ui.components.ui.JsonTree
...
private fun RequestLogDetail(log: LogEntry.RequestLog) {
    ...
    val tabs = remember { listOf("概览", "头部", "请求体", "响应体") }
    ...
    Column(modifier = Modifier.fillMaxWidth().fillMaxHeight()) {
        ...
        HorizontalPager(
            ...
            when (page) {
                0 -> OverviewContent(log = log, dateFormat = dateFormat)
                1 -> CombinedHeaderContent(
                    requestHeaders = log.requestHeaders,
                    responseHeaders = log.responseHeaders,
                )
                2 -> BodyContent(body = log.requestBody, emptyHint = "无请求体")
                3 -> BodyContent(body = log.responseBody, emptyHint = "无响应体")
            }
        }
    }
}
...
@Composable
private fun CombinedHeaderContent(
    requestHeaders: Map<String, String>,
    responseHeaders: Map<String, String>,
) {
    val hasRequest = requestHeaders.isNotEmpty()
    val hasResponse = responseHeaders.isNotEmpty()

    if (!hasRequest && !hasResponse) {
        EmptyPage(hint = "无请求头与响应头")
        return
    }
    ...
    SelectionContainer {
        Column(
            ...
            if (hasRequest) {
                ...
                requestHeaders.forEach { (key, value) -> HeaderItem(key, value) }
            }
            if (hasResponse) {
                if (hasRequest) {
                ...
                responseHeaders.forEach { (key, value) -> HeaderItem(key, value) }
            }
        }
    }
}
...
@Composable
private fun OverviewContent(log: LogEntry.RequestLog, dateFormat: SimpleDateFormat) {
    SelectionContainer {
        Column(
            modifier = Modifier
                .fillMaxWidth()
...
@Composable
private fun BodyContent(body: String?, emptyHint: String) {
    if (body.isNullOrBlank()) {
        EmptyPage(hint = emptyHint)
        return
    }
...
@Composable
private fun EmptyPage(hint: String) {
    Box(
        modifier = Modifier
            .fillMaxWidth()
            .fillMaxHeight(),
