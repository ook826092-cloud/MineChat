...
import cn.mine.minestars.ui.components.richtext.HighlightCodeVisualTransformation
...
import cn.mine.search.SearchResult
...
fun SettingSearchDetailPage(
    ...
    searchServiceDAO: SearchServiceDAO = koinInject(),
) {
    ...
    val service = searchServices.find { it.id == serviceId } ?: return
    var options by remember(service) { mutableStateOf(service) }

    fun save(updated: SearchServiceOptions) {
        options = updated
        scope.launch {
            searchServiceDAO.insert(updated.toSearchServiceEntity())
        }
    }
    ...
    Scaffold(
                ...
                actions = {
                    if (searchServices.size > 1) {
                        IconButton(
                            onClick = {
                                scope.launch {
                                    searchServiceDAO.deleteById(serviceId.toString())
                                }
                                nav.popBackStack()
                            }
                        ) {
        ...
        LazyColumn(
            ...
            item("test") {
                SearchTestSection(
                    options = options,
                    commonOptions = aiSettings.searchCommonOptions
                )
            }
        }
    }
...
private fun SearchServiceOptionsEditor(
    ...
    when (options) {
        ...
        is SearchServiceOptions.MineChatOptions -> {
            MineChatOptions(options) { onUpdateOptions(it) }
        }
...
@Composable
private fun MineChatOptions(
    options: SearchServiceOptions.MineChatOptions,
    onUpdateOptions: (SearchServiceOptions.MineChatOptions) -> Unit
) {
    FormItem(
        label = {
            Text(stringResource(R.string.search_detail_api_key))
