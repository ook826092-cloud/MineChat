package cn.mine.minestars.core.workspace
...
class WorkspaceManager(
    ...
    private val prootShellRunner: ProotShellRunner? = null,
    private val bindMounts: List<BindMount> = emptyList(),
) {
    ...
    fun rootfsLocation(root: String): RootfsLocation? {
        if (!hasRootfs(root)) return null
    ...
    fun resolveRootfsPath(root: String, relPath: String): File {
        val base = linuxDir(root)
        ...
        require(target.path == base.path || target.path.startsWith(base.path + File.separator)) {
            "Path escapes rootfs: $relPath"
        }
        return target
    }

    fun rootfsFileSize(root: String, path: String): Long {
        val file = resolveRootfsPath(root, path)
        require(file.exists()) { "Rootfs file does not exist: $path" }
    ...
    fun executeCommand(
        ...
        val runner = if (prootShellRunner != null && hasRootfs(root)) {
            prootShellRunner
        } else {
            shellRunner
        }
