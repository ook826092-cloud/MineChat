                ...
                when (deltaPart) {
                    ...
                    is UIMessagePart.Reasoning -> {
                        ...
                        if (deltaPart.reasoning.isEmpty() && deltaPart.metadata == null) {
                            ...
                            } else if (acc.any { it is UIMessagePart.Text }) {
                                // 正文已开始：后续 reasoning_content 是服务端把完整思考重发了一遍（如 NVIDIA 流式末尾），丢弃以避免重复渲染
                                acc
                            } else {
